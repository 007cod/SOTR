...
from net import gtnet, GCNet, GCNetG, GCNetSA
...
parser.add_argument('--model',type=str,default='GCNet',help='mode name')
...
def main(runid):
    ...
    dataloader = load_dataset("data/"+args.data, args.batch_size, args.batch_size, args.batch_size)
    ...
    if args.model == "wavenet":
        model = wavenet(device, args.num_nodes, args.dropout, in_dim=args.in_dim,out_dim=args.seq_out_len )
    elif args.model == "GCNet":
        model = GCNet(args.gcn_true, args.buildA_true, args.gcn_depth, args.num_nodes,
                  device, 
                  dropout=args.dropout, subgraph_size=args.subgraph_size,
                  node_dim=args.node_dim,
                  conv_channels=args.conv_channels, end_channels= args.end_channels,
                  seq_length=args.seq_in_len, in_dim=args.in_dim, out_dim=args.seq_out_len,
                  layers=args.layers, propalpha=args.propalpha, tanhalpha=args.tanhalpha)
    elif args.model == "GCNetG":
        model = GCNetG(args.gcn_true, args.buildA_true, args.gcn_depth, args.num_nodes,
                device, 
                dropout=args.dropout, subgraph_size=args.subgraph_size,
                node_dim=args.node_dim,
                conv_channels=args.conv_channels, end_channels= args.end_channels,
                seq_length=args.seq_in_len, in_dim=args.in_dim, out_dim=args.seq_out_len,
                layers=args.layers, propalpha=args.propalpha, tanhalpha=args.tanhalpha)
    elif args.model == "GCNetSA":
        model = GCNetSA(args.gcn_true, args.buildA_true, args.gcn_depth, args.num_nodes,
                device, 
                dropout=args.dropout, subgraph_size=args.subgraph_size,
                node_dim=args.node_dim,
                conv_channels=args.conv_channels, end_channels= args.end_channels,
    ...
    for i in range(1,args.epochs+1):
        ...
        if mvalid_loss<minl:
            torch.save(engine.model.state_dict(), args.save + '/'+ args.model + "exp" + str(args.expid) + "_" + str(runid) +".pth")
            minl = mvalid_loss
    ...
    engine.model.load_state_dict(torch.load(args.save + '/'+ args.model + "exp" + str(args.expid) + "_" + str(runid) +".pth", weights_only=False))
...
def tmain():
    args.save = "./save/" + args.data
    f = open(args.save + f"/{args.model}_log.txt", 'w')
    sys.stdout = f
    vmae = []
    vmape = []
    vrmse = []
    ...
    with open(args.output_path, "a") as f:
        print('model\tdata\ttest|horizon\tMAE-mean\tRMSE-mean\tMAPE-mean\tMAE-std\tRMSE-std\tMAPE-std')
        for i in [2,5,11]:
            log = '{}\t{}\t{:d}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}'
            print(log.format(args.model, args.data, i+1, amae[i], armse[i], amape[i], smae[i], srmse[i], smape[i]))
            f.write(log.format(args.model, args.data, i+1, amae[i], armse[i], amape[i], smae[i], srmse[i], smape[i])+"\n")


if __name__ == "__main__":
    args.output_path = "/data/chenjinwen/cjw/RecommendSC/MTGNN/save/ans3.txt"
    with open(args.output_path, "a") as f:
        f.write('model\tdata\ttest|horizon\tMAE-mean\tRMSE-mean\tMAPE-mean\tMAE-std\tRMSE-std\tMAPE-std\n')
    for mode_name in ["GCNetG", "GCNetSA"]:  #"GCNetSA", "GCNetG", "GCNet", "wavenet"
        for data_name in ["PEMS04", "PEMS08"]:
            if data_name == "PEMS08":
                args.num_nodes = 170
            else:
                args.num_nodes = 307
            args.model = mode_name
            args.data = data_name
            tmain()
